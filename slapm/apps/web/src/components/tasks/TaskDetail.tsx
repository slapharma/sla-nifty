...
export function TaskDetail({ task, onClose, onSubtaskClick }: Props) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-[420px] bg-white shadow-xl border-l border-gray-200 flex flex-col z-50">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        {/* Subtasks — hidden for subtasks (no sub-sub-tasks) */}
        {!isSubtask && (
          <div>
            ...
            <div className="space-y-1">
              {(task.subtasks ?? []).map((sub) => (
                ...
                >
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); updateStatus.mutate({ taskId: sub.id, status: sub.status === 'DONE' ? 'TODO' : 'DONE' }) }}
                  ...
                  <button
                    ...
                    className={`text-sm flex-1 text-left hover:underline ${sub.status === 'DONE' ? 'line-through text-slate-400' : 'text-slate-700'}`}
                  >
                    {sub.title}
                  </button>
                </div>
      ...
      <div className="px-6 py-4 border-t border-gray-100">
        {task.status !== 'DONE' ? (
          <button
            onClick={() => { updateStatus.mutate({ taskId: task.id, status: 'DONE' }); onClose() }}
          ...
          <button
            onClick={() => { updateStatus.mutate({ taskId: task.id, status: 'TODO' }); onClose() }}
            className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium transition-colors"
          >
            Reopen Task
          </button>
        )}
